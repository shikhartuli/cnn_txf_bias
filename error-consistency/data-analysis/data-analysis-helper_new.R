...
HUMAN.DATA.NAME = "Humans (avg.)"
human.avg = list(name="Humans (avg.)",
                 color=human.100,
...
plot.consistency = function(dat, method=c("consistency", "kappa"),
  ...
  max.consistency = -Inf
  max.consistent.models = NULL
  if(length(PLOT.SUBJECTS_CNN) > 3) {
    cnn.to.cnn.cex = points.cex-0.4 # smaller symbols for more models
  } else {
    cnn.to.cnn.cex = points.cex
  }
  i = 1
  for(subj1 in PLOT.SUBJECTS_CNN) {
    j = 1
    for(subj2 in PLOT.SUBJECTS_CNN) {
      if(i>j) {
  ...
  # ViT-vs-ViT consistency
  vit.data = data.frame()
  max.consistency = -Inf
  max.consistent.models = NULL
  if(length(PLOT.SUBJECTS_ViT) > 3) {
    cnn.to.cnn.cex = points.cex-0.4 # smaller symbols for more models
  } else {
    cnn.to.cnn.cex = points.cex
  }
  i = 1
  for(subj1 in PLOT.SUBJECTS_ViT) {
    j = 1
    for(subj2 in PLOT.SUBJECTS_ViT) {
      if(i>j) {
        if(grepl("resnet", subj1$data.name) & grepl("resnet", subj2$data.name) |
           grepl("densenet", subj1$data.name) & grepl("densenet", subj2$data.name) |
           grepl("squeezenet", subj1$data.name) & grepl("squeezenet", subj2$data.name) |
        ...
        p = data.frame(x=consistency[x.value.name][1,1],
                       y=consistency[y.value.name][1,1],
                       pch=pch, col=col, bg=blk,
                       cex=cnn.to.cnn.cex)
        vit.data = rbind(vit.data, p)
        if(consistency[y.value.name][1,1] > max.consistency) {
          max.consistency = consistency[y.value.name][1,1]
          max.consistent.models = paste(subj1$data.name, subj2$data.name)
        }
  ...
  min.value = min(min(human.data$y), min(cnn.data$y), min(cnn.human.data$y), min(vit.data$y))
  max.value = max(max(human.data$y), max(cnn.data$y), max(cnn.human.data$y), max(vit.data$y))
  ...
  combined.data = rbind(human.data, cnn.data, vit.data, cnn.human.data)
...
plot.shape.bias = function(dat, x.is.content=TRUE,
  ...
  if(plot.legend) {
    ...
    add.legend(subjects, legend.position = legend.position, inset = c(0.25, 0),
               cex=0.8, pt.cex=1.5, y=legend.y)
  }
}
